...
def view_entry(entry_content):
    ...
    clouds_label.image = clouds_image  # Keep a reference to avoid garbage collection
...
def display_entries(diary):
    ...
    for entry in diary:
        ...
        listbox.insert(tk.END, "-" * 50)  # Add a divider line

    def on_listbox_select(event):
        selected_index = listbox.curselection()
        if selected_index:
            # Adjust index to skip dividers
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.diary = []  # This will hold the diary entries
